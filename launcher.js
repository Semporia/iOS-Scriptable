...
// icon-color: deep-purple; icon-glyph: link;

/**
 * 作者: Semporia
 * 版本: 1.0.0
 * 更新时间：2020-12-16
 * github: https://github.com/Semporia/scriptable
 */
...
var Stack = ({children, ...props}) => {
  ...
  const _children = Array.isArray(children) ? children.map((child) => {
...
var Launcher = class {
  ...
  async render() {
    if (isLaunchInsideApp()) {
      await showNotification({title: "稍等片刻", body: "小組建正在渲染中...", sound: "alert"});
    }
    ...
    }, size === "small" && this.renderSmall(), size === "medium" && this.renderMedium(), size === "large" && this.renderLarge());
  }
  renderSmall() {
    return  h("wstack", {
      flexDirection: "column"
    },  h("wstack", null,  h(Grid, {
      ...this.config[0]
    })));
  }
  renderMedium() {
    return  h("wstack", null,  h(Grid, {
      ...this.config[0]
    }),  h("wstack", {
      flexDirection: "column"
    },  h("wstack", null,  h(Grid, {
